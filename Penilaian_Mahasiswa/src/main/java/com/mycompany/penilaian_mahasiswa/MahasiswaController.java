...
public class MahasiswaController implements Initializable {
    ...
    // Menginisialisasi komponen yang ada di dalam FXML
    @Override
    public void initialize(URL url, ResourceBundle rb) {
        System.out.println("Controller Mahasiswa siap!");
        // Anda bisa memuat data mahasiswa ke TableView di sini jika diperlukan
    }
    ...
    try (Connection connection = KoneksiDB.getConnection()) {
        connection.setAutoCommit(false);  // Menonaktifkan autocommit untuk transaksi manual
        ...
        try (PreparedStatement pst = connection.prepareStatement(query)) {
            ...
            if (result > 0) {
                ...
                connection.rollback();  // Rollback jika gagal
                return false;
            }
